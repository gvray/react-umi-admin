...
| ✅ **request 插件（封装的** `umi-request`**）** | 封装层太重、依赖旧版 fetch polyfill，灵活性差 | 替换为 axios + 自定义 hooks（如 `@gvray/request`） |
| ✅ **@/models 全局状态管理** | 适合老项目或约定式开发，但模型共享复杂，异步逻辑不如 Zustand 明确 | 中大型项目建议直接用 Zustand |
...
| ✅ **内置 DVA 支持** | DVA 学习成本高，异步和副作用处理陈旧，现代项目极少使用 | 推荐 Zustand 等现代状态库 |
